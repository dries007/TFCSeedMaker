package net.dries007.tfc.seedmaker.util;

import java.awt.*;
import java.util.Arrays;
import java.util.HashSet;
import java.util.List;

/**
 * @author Dries007
 */
public enum Biome
{
    OCEAN(0,            new Color(0x3A3CEC)),
    PLAINS(1,           new Color(0x4AA02C)),
    LAKE(2,             new Color(0x66B2EC)),
    HIGH_HILLS(3,       new Color(0xC5BD06)),
//    FOREST(4),
//    TAIGA(5),
    SWAMPLAND(6,        new Color(0x158864)),
    RIVER(7,            new Color(0x00B1AF)),
//    HELL(8),
//    SKY(9),
//    FROZENOCEAN(10),
//    FROZENRIVER(11),
//    ICEPLAINS(12),
//    ICEMOUNTAINS(13),
//    MUSHROOMISLAND(14),
//    MUSHROOMISLANDSHORE(15),
    BEACH(16,           new Color(0x8D6AA1)),
    GRAVEL_BEACH(17,    new Color(0x821CA1)),
//    FORESTHILLS(18),
//    TAIGAHILLS(19),
    HIGH_HILLS_EDGE(20, new Color(0xC47D21)),
//    JUNGLE(21),
//    JUNGLEHILLS(22),
//    JUNGLEEDGE(23),
//    DEEPOCEAN(24),
//    STONEBEACH(25),
//    COLDBEACH(26),
//    BIRCHFOREST(27),
//    BIRCHFORESTHILLS(28),
//    ROOFEDFOREST(29),
    ROLLING_HILLS(30,   new Color(0x9E5569)),
    MOUNTAINS(31,       new Color(0x940007)),
    MOUNTAINS_EDGE(32,  new Color(0x7A0040)),
//    MEGATAIGAHILLS(33),
//    EXTREMEHILLSPLUS(34),
    HIGH_PLAINS(35,     new Color(0x348017)),
    DEEP_OCEAN(36,      new Color(0x000059));
//    MESA(37),
//    MESAPLATEAU_F(38),
//    MESAPLATEAU(39);

    public final int id;
    public final Color color;

    Biome(final int id, final Color color)
    {
        this.id = id;
        this.color = color;
    }

    public static final List<Biome> SPAWNLIST = Arrays.asList(/*FOREST, */PLAINS, ROLLING_HILLS, SWAMPLAND, MOUNTAINS, HIGH_PLAINS);
    public static final List<Biome> ALLOWEDBIOMES = Arrays.asList(OCEAN, HIGH_HILLS, PLAINS, HIGH_PLAINS, SWAMPLAND, ROLLING_HILLS, MOUNTAINS);
    public static final Biome LIST[] = new Biome[256];
    public static final Color COLORS[] = new Color[256];

    static
    {
        HashSet<Integer> pool = new HashSet<>();
        for (Biome biome : values())
        {
            if (!pool.add(biome.id)) throw new RuntimeException("Duplicate Biome");
            LIST[biome.id] = biome;
            COLORS[biome.id] = biome.color;
        }
    }

    public static boolean isOceanicBiome(int id)
    {
        return id == OCEAN.id || id == DEEP_OCEAN.id;
    }

    public static boolean isMountainBiome(int id)
    {
        return id == MOUNTAINS.id || id == MOUNTAINS_EDGE.id;
    }

    public static boolean isBeachBiome(int id)
    {
        return id == BEACH.id || id == GRAVEL_BEACH.id;
    }
}
